...
const LEFT_RAYCAST_NAME = "LeftTouchRayCast"
const RIGHT_RAYCAST_NAME = "RightTouchRayCast"
...
func _on_tracker_enabled():
	print("Loading input map for " + _get_tracker_label() + " with device id: " + str(get_joystick_id()))
	_load_tracked_hand_input_map()


func _on_tracker_disabled():
	print("Clearing input map for " + _get_tracker_label() + " with device id: " + str(get_joystick_id()))
	_clear_tracked_hand_input_map()


func _load_tracked_hand_input_map():
	# Clear the map first
	_clear_tracked_hand_input_map()
	
	var raycast_name = _get_controller_raycast_name()
	
	# Load the click action
	var click_action = raycast_name + "_click"
	InputMap.add_action(click_action, 0.0)
	
	var xa_button_click = InputEventJoypadButton.new()
	xa_button_click.device = get_joystick_id()
	xa_button_click.button_index = CONTROLLER_BUTTON.XA
	InputMap.action_add_event(click_action, xa_button_click)
	
	var trigger_button_click = InputEventJoypadButton.new()
	trigger_button_click.device = get_joystick_id()
	trigger_button_click.button_index = CONTROLLER_BUTTON.INDEX_TRIGGER
	InputMap.action_add_event(click_action, trigger_button_click)
	
	
	# Load the left scroll action
	var left_scroll_action = raycast_name + "_left_scroll"
	InputMap.add_action(left_scroll_action, 0.0)
	
	var left_scroll_motion = InputEventJoypadMotion.new()
	left_scroll_motion.device = get_joystick_id()
	left_scroll_motion.axis = JOY_ANALOG_LX
	left_scroll_motion.axis_value = -1.0
	InputMap.action_add_event(left_scroll_action, left_scroll_motion)
	
	
	# Load the right scroll action
	var right_scroll_action = raycast_name + "_right_scroll"
	InputMap.add_action(right_scroll_action, 0.0)
	
	var right_scroll_motion = InputEventJoypadMotion.new()
	right_scroll_motion.device = get_joystick_id()
	right_scroll_motion.axis = JOY_ANALOG_LX
	right_scroll_motion.axis_value = 1.0
	InputMap.action_add_event(right_scroll_action, right_scroll_motion)
	
	
	# Load the up scroll action
	var up_scroll_action = raycast_name + "_up_scroll"
	InputMap.add_action(up_scroll_action, 0.0)
	
	var up_scroll_motion = InputEventJoypadMotion.new()
	up_scroll_motion.device = get_joystick_id()
	up_scroll_motion.axis = JOY_ANALOG_LY
	up_scroll_motion.axis_value = -1.0
	InputMap.action_add_event(up_scroll_action, up_scroll_motion)
	
	
	# Load the down scroll action
	var down_scroll_action = raycast_name + "_down_scroll"
	InputMap.add_action(down_scroll_action, 0.0)
	
	var down_scroll_motion = InputEventJoypadMotion.new()
	down_scroll_motion.device = get_joystick_id()
	down_scroll_motion.axis = JOY_ANALOG_LY
	down_scroll_motion.axis_value = 1.0
	InputMap.action_add_event(down_scroll_action, down_scroll_motion)


func _clear_tracked_hand_input_map():
	var raycast_name = _get_controller_raycast_name()
	
	# Clear the click action
	_clear_action(raycast_name  + "_click")
	
	# Clear the left scroll action
	_clear_action(raycast_name + "_left_scroll")
	
	# Clear the right scroll action
	_clear_action(raycast_name + "_right_scroll")
	
	# Clear the up scroll action
	_clear_action(raycast_name + "_up_scroll")
	
	# Clear the down scroll action
	_clear_action(raycast_name + "_down_scroll")


func _clear_action(action: String):
	if (InputMap.has_action(action)):
		InputMap.erase_action(action)

func _get_controller_raycast_name():
	return LEFT_RAYCAST_NAME if (controller_id == LEFT_TRACKER_ID) else RIGHT_RAYCAST_NAME
